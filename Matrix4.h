...
class Matrix4
	...
	const static Matrix4 TranslationMultMat(Cartesian3 translation);
	const static Matrix4 ScaleMultMat(float xScale, float yScale, float zScale);
	const static Matrix4 ScaleMultMat(float scale) { return ScaleMultMat(scale, scale, scale); };
	const static Matrix4 RotationMultMat(const Cartesian3 axis, float theta);
	}; // Matrix4
