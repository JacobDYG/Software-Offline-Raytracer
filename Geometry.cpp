...
bool Triangle::geometricIntersection(Ray ray, float& tOut)
{
    ...
    if (std::fabs(planeNDotRayD) < 1e-8) return false; // 0 or very close, therefore consider parallel
...
bool Triangle::intersection(Ray ray, float& tOut, float& uOut, float& vOut)
{
    ...
    if (std::fabs(determinant) < 1e-8) return false; // epsilon for numerical error
